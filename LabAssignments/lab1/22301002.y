...
int yyparse(void);        /* functiong doing parsing, when parser needs token it generates the lexar*/
int yylex(void);          /* function doing lexical analysis */

extern FILE *yyin;        /*file type pointer to access files*/
...
%token IF ELSE FOR WHILE    /* defining tokens written in lex file */
%token INT FLOAT VOID
%token ADDOP MULOP ASSIGNOP LOGICOP RELOP
%token LPAREN RPAREN
%token LCURL RCURL
%token LTHIRD RTHIRD
...
%token SWITCH CASE DEFAULT GOTO CHAR DOUBLE
...
%%
/* driver code */
int main(int argc, char *argv[])      /* counts arguments given, increasing the counter. *argv[] -> argument vector that holds the arguments, indexing starts at 0, idx holds input file name*/
{
	if(argc != 2) 
	{
	...
	yyin = fopen(argv[1], "r");  /* opened with read mode*/
